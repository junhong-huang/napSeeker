...
double readPEBamToBed6MapNew(BamReader &reader, chromBed6Map &bed6Hash, int maxInsertLen, int skipSplice, int ccaTail)
// must at same chromosome
{
  int i            = 0;
  double totalNum  = 0;
        ...
        else
        {
          ...
          if (ccaTail == 1)
          {
            ...
            if (bam1.IsReverseStrand() == true) {
              ...
              if (readSeq.rfind(minusStr, 0) != 0) continue;
            }
            else
            {
              string readSeq  = bam2.QueryBases;
              ssize_t idx = readSeq.size() - 3;
              if (readSeq.rfind(plusStr, idx) != idx) continue;
            }
          }
          string chrom1 = refs.at(bam1.RefID).RefName;
          string chrom2 = refs.at(bam2.RefID).RefName;
...
double readBamToBed6Map(BamReader &reader, chromBed6Map &bed6Hash, int keepDup, int maxInsertLen, int skipSplice, int ccaTail)
{
  int i           = 0;
  char delims[]   = "-";
  int fieldNum    = 0;
